...
namespace DynStack.SimulationRunner.Common {
  public abstract class SimulationHost {
            ...
            if (!options.RunSync) {
              ...
              while (remaining > 0) {
                ...
                remaining = Outgoing.Count;
                            ...
                            INetMQSocket requestSocket = new DealerSocket();
                            requestSocket.Bind(options.SyncURL);
                            for (int i=0; i<options.NumOfReps;i++) {
                                result = RunSimulation2(await _settingsReceived.Task, requestSocket, options.Id);
                            }
            }
          } finally {
            poller.Stop();
   ...
   protected abstract bool RunSimulation2(byte[] settingsBuf, INetMQSocket requestSocket, string id, bool simulateAsync = true, bool useIntegratedPolicy = false);
